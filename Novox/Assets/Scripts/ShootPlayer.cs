...
public class ShootPlayer : MonoBehaviour {
	...
	void Awake () {
		...
		StartPosition = spring.connectedBody.transform;
		
	}
	
	void Start () {
		StringSetup ();
		mouseRay = new Ray(StartPosition.position, Vector3.zero);
	...
	void Dragging () {
		...
		Vector2 fromStartPosition = mousePos - StartPosition.position;
		
		if (fromStartPosition.sqrMagnitude > stretchSquare) {
			mouseRay.direction = fromStartPosition;
